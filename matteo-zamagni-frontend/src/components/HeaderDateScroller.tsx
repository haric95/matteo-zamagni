    ...
    scrollerAvailableYears && (
      ...
      >
            ...
            <div className="wheel__slides" style={{ width: 0 }}>
              {slideValues().map(({ style, value }, idx) => (
                <div className="wheel__slide" style={style} key={idx}>
                  <span
                    className="translate-x-1/2 dark:text-offWhite text-black transition-all duration-500 text-xs"
                    onClick={(e) => {
                      e.preventDefault();
        ...
        <div className="absolute top-0 w-full h-full flex justify-center pointer-events-none">
          <div className="w-16 border-x-[1px] dark:border-fadedWhite border-black transition-all duration-500"></div>
        </div>
        <AnimatePresence>
          {currentYearIndex !== 0 && (
            <motion.button
